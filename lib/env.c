...
int signal[10] = {0};
struct Env_list env_wait_queue1;
struct Env_list env_wait_queue2;
...
//		wait_queue[qtail] = e;
		if (s == 1)
			LIST_INSERT_TAIL(&env_wait_queue1, e, env_link);
		else if (s == 2)
			LIST_INSERT_TAIL(&env_wait_queue2, e, env_link);
		e -> status = 1;
//		qtail = (qtail + 1) % 1024;
	}
	return 0;
...
int envid2env(u_int envid, struct Env **penv, int checkperm)
{
    ...
    if (e->env_status == ENV_FREE || e->env_id != envid) { // Pre-Condition
        *penv = 0;
        return -E_BAD_ENV;
    }
    /* Hints:
...
env_init(void)
{
	...
	LIST_INIT(&env_wait_queue1);
	LIST_INIT(&env_wait_queue2);
